...
//Ask user for password, password is "awesome"
char good [8]= "awesome";
void UART_password(void) {
  ...
  while(1){	
		...
		//compare if the inputed string is same as the good password
		result = strcmp (good, string );
		if (result == 0){
			UART_OutString(" correct password, Enjoy the game!");
			UART_OutChar(CR);UART_OutChar(LF);
			break;
		}
		else{ 
			UART_OutString("wrong password, please try again");
			UART_OutChar(CR);UART_OutChar(LF);
		}
	}
}
